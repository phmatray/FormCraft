...
- **A hidden collection field no longer blocks submit, and correcting a row keeps the collection's summary message in sync (#342).** `ValidateModelAsync`'s collection loop skipped the visibility check the ordinary-field loop already had, so a collection with `IsVisible = false` and an invalid item made the whole form report invalid with no control on screen to explain why - the submit button silently stopped working. The collection loop now skips a hidden collection exactly like an ordinary hidden field. Separately, correcting one cell used to clear only the nested `Items[i].Field` message, leaving that row's line in the collection's flat `ValidationSummary` message until the next full submit; it is now removed at the same time, without revalidating any other row. **If a form relied on the old behaviour** - a hidden collection with an invalid item blocking submit - that form's behaviour changes: submit now succeeds.
...
- **Required fields are announced to screen readers again — and no longer carry the HTML5 `required` attribute (#263).** FormCraft's convention has always been that `Required()` adds validation, not browser constraint validation; forms render `novalidate` and messages come from your validators. #199 had to bend that: MudBlazor derived the HTML5 `required` attribute **and** `aria-required` from a single `Required` flag and wrote both *after* splatting caller attributes, so the accessibility annotation could not be had without the attribute. Announcing the field won, and the HTML5 attribute came along.
  ...
  ⚠️ **The visible asterisk goes with it.** MudBlazor draws its `*` from the same `Required` parameter as the HTML5 attribute, and those two are still fused, so a plain `.Required("…")` field no longer shows one. This was a deliberate trade rather than an oversight: **`.WithNativeRequired()` restores MudBlazor's native decoration — asterisk and HTML5 attribute together** — on any field you want marked visibly. If you relied on the asterisk appearing automatically for required fields, add that call, or style the requirement yourself from your own label markup.
...
- **`.editorconfig` is now actually enforced, and the tree matches it (#301).** The file declared its code-style rules at `warning` severity and `Directory.Build.props` sets `TreatWarningsAsErrors=true` — which together *look* like enforcement and were not: `IDE*` analyzers only run at build time when `EnforceCodeStyleInBuild` is set, and it wasn't, while no CI job ran `dotnet format` either. Nothing anywhere read those severities, so 574 violations had accumulated across 201 files. There is now a Nuke `Format` target (`./build.sh Format`) wrapping `dotnet format --verify-no-changes`, wired into `ci.yml` ahead of the test run, and a one-off pass has cleared the backlog — split into a whitespace commit and a code-style commit so the shape-changing fixes stayed reviewable. Enforcement lives in CI rather than the build on purpose: with warnings-as-errors, build-time style analysis would break `dotnet build` mid-edit over a missing brace.
  ...
  ⚠️ **`dotnet format` can corrupt multi-targeted files when it applies fixes** — `FormCraft` is `net8.0;net10.0`, and a fix applied once per target framework can land as a literal `<<<<<<< TODO: Unmerged change` conflict block written into the `.cs` file, which then doesn't compile. Verify mode never writes, so the CI gate is unaffected. **`FormCraft.UnitTests/Ci/ConflictMarkerTests` (#347) is the guard** — it fails `dotnet test` and names the offending file and line if one is committed. `grep -rl '<<<<<<< TODO' --include='*.cs' .` after an apply run is still a useful quick diagnostic, but the test is what actually enforces this.
...
- **A field component now re-reads its configuration when it is handed a different field — in both adapters (#298, #335).** Every field component read its settings once, in `OnInitialized`, and never looked again. Blazor reuses a component instance whenever the render-tree shape matches, so an instance could be given a different field while those cached attributes still described the previous one — and it would go on rendering that field's mask, adornment, input type, numeric bounds and select options indefinitely. Nothing threw and nothing logged; the field simply showed the wrong thing, plausibly enough that it read as correct.
  ...
  **How you hit it:** by **swapping `FormCraftComponent.Configuration`** on a live form — a wizard step, a mode toggle, anything that renders a different form over the same component tree.
  ...
  Components now load their configuration in an `OnFieldConfigurationChanged()` hook, called on first render and again whenever the field changes — guarded by a reference comparison on the field itself, so an ordinary re-render (which happens on every keystroke) costs one comparison rather than a re-read of every attribute. State derived from the configuration is reset with it, including the password-visibility toggle: a revealed password on the old field must not leave the new field's secret rendered in clear text.

  **The hook lives on `FieldComponentBase` in `FormCraft` core**, so both the MudBlazor and Fluent UI adapters inherit it and a custom component gets it wherever it sits. It started inside the MudBlazor package and had to be wired three times there — that package has three component hierarchies — which was the signal it belonged a layer down; #335 moved it before the Fluent adapter could become a fourth copy, the drift this library has already paid for in #146, #177, #184, #190, #203 and #279.

  **Collection rows (MudBlazor) now keep their component identity across an add, remove or reorder (#334).** The item loop used to match rows by position, so removing one re-pointed each surviving component at its neighbour's data. Displayed values were unaffected — those reload from the model — but component *identity* was not, and with it any state the value does not restore: a revealed password, MudBlazor's own input state, the masked-value diagnostic latch (#283). Keying the loop on the item itself was tried and reverted (#308): Blazor compares keys with `Equals`, and item types are constrained only to `new()`, so two `record` or `struct` rows with equal content are a duplicate key and the render throws — on a record-typed item form, adding two empty rows was enough. The fix keys each row on a fresh, per-item token from a `ConditionalWeakTable` instead of the item itself, so identity follows the item without ever comparing it: two equal-by-value rows still get two different tokens, and a value-typed item — which has no reference-stable identity to give the table — falls back to today's positional behaviour rather than crashing. The Fluent UI adapter's collection has the same unkeyed loop and is tracked separately.
...
- **`.WithMask(...)` — a typed builder for masks, replacing the `"Mask"` magic string.** The entry above made masks work; the only way to reach one was still `.WithAttribute("Mask", "…")`, which is undiscoverable, unchecked by the compiler, and one typo away from silently doing nothing — the defect #204 closed for `.WithNativeRequired()`. There is now a typed builder, and it reaches two configurations the string never could (#265)
  ...
  **A factory-produced mask whose pattern is computed lazily used to be discarded too — silently, even through the new overload.** `BlockMask` builds its `Mask` string on first use, not at construction, so `.WithMask(() => new BlockMask(...))` resolved a *fresh* instance whose `Mask` read as blank and got rejected by the same rule that refuses `PatternMask("")`. The field rendered completely unmasked, with no error or warning. That rule is now scoped to `PatternMask` alone — the only implementation whose pattern is eager and whose blank pattern genuinely means "no mask" — so a factory-produced `BlockMask` or `RegexMask` is bound whatever it reports at construction (#317). `MultiMask` and `DateMask` are unaffected either way: both derive from `PatternMask` and set their pattern just as eagerly, so a blank one was, and still is, correctly read as no mask.
...
- **`dotnet test` now names a committed merge-conflict marker instead of leaving it to a bare compiler error (#347).** `dotnet format` in apply mode computes a fix once per target framework, and on this repo's multi-targeted projects (`net8.0;net10.0`) a disagreement between the two can be written as a literal conflict block into the `.cs` file instead of failing — a file that then does not compile (`CS8300`, which already fails a full solution build). `FormCraft.UnitTests/Ci/ConflictMarkerTests` scans every tracked `.cs` file for a conflict marker at the start of a line and fails with the offending file and line if it finds one — a friendlier signal than a raw compiler error, and one that also catches it in a narrower local run (e.g. `dotnet test FormCraft.UnitTests` alone) that never builds the sibling project the corruption actually landed in, which is exactly what happened across #301 and #307.
