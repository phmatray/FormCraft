...
/// <list type="bullet">
/// <item><description><see cref="OrderItem"/> — <c>string</c>, rendered by a text field.</description></item>
/// <item><description><see cref="BasketLine"/> — <c>int</c>, rendered by a numeric field.</description></item>
/// <item><description><see cref="AppointmentSlot"/> — <c>DateTime</c>, rendered by a date field.</description></item>
/// <item><description><see cref="BasketLine"/> again — <c>bool</c>, rendered by a checkbox,
/// which some adapters bind neither adornments nor <c>Required</c> for, and where those are
/// therefore pinned as inert.</description></item>
/// <item><description><see cref="PricedLine"/> — <c>decimal</c>, rendered by a decimal-typed
/// numeric field. A distinct closed generic from the <c>int</c> one, and the only one where
/// culture-sensitive parsing is observable (#218, #258).</description></item>
/// </list>
...
/// went through a hand-written <c>RenderTreeBuilder</c> in the MudBlazor adapter's collection
/// component, with a shared helper feeding the first three and a bespoke method for the fourth — so
/// a suite could pass for a standalone field and fail for the same field in an item form. #203
/// deleted that renderer: every field now goes through <c>IFieldRendererService</c> and the same
/// per-type component regardless of placement. The groupings above survive because they are still
/// distinct <i>components</i>, but they are no longer distinct <i>paths</i>, and a suite built on
/// this fixture is now checking that the item placement keeps inheriting the component's behaviour
/// rather than that a second implementation matches it. The decimal pair was added in #258 under
/// that later reading — not a fifth path, a fifth component a real suite needed to name.
/// </para>
/// <para>
/// <b>Shared across adapters since #343.</b> Originally MudBlazor-only, extracted here so the Fluent
/// UI adapter's test suite draws from the same models instead of re-declaring them under its own
/// vocabulary — the exact drift #205/#258/#282 fought inside one assembly, reproduced across two.
/// Nothing in this file names a UI-framework type; the rendering half
/// (<c>CollectionItemFixtureRenderExtensions.RenderItemForm</c>) is bUnit/MudBlazor-flavoured and
/// stays behind in <c>FormCraft.ForMudBlazor.UnitTests</c> — each adapter suite keeps its own render
/// helper.
/// </para>
/// <para>
...
public static class CollectionItemFixture
{
    ...
    public static OrderModel NewOrder(string productName = "") =>
    ...
    /// </remarks>
    public static OrderModel NewOrderWithItems(params string[] productNames)
    {
    ...
    public static BasketModel NewBasket(int quantity = 0, bool isGift = false) =>
    ...
    public static AppointmentModel NewAppointment(DateTime when = default) =>
    ...
    public static PricedBasketModel NewPricedBasket(decimal price = 0m) =>
    ...
    /// </summary>
    public static NamedOrderModel NewNamedOrder(string name = "", string itemName = "") =>
    ...
    /// </param>
    public static IFormConfiguration<OrderModel> TextItemForm(
    ...
    /// </param>
    public static IFormConfiguration<BasketModel> NumericItemForm(
    ...
    /// form. See <see cref="TextItemForm"/> for the full contract (#300).
    /// </param>
    public static IFormConfiguration<AppointmentModel> DateItemForm(
    ...
    /// component type that (in the MudBlazor adapter) binds neither adornments nor <c>Required</c>.
    /// </summary>
    /// <param name="configure">Configures the item's field. Runs after its default label, so it can override it.</param>
    /// <param name="configureCollection">
    /// Configures the <i>collection</i> rather than a field; runs last, after the label and item
    ...
    /// </param>
    public static IFormConfiguration<BasketModel> BooleanItemForm(
    ...
    /// component. A decimal-typed numeric field is a different closed generic from an <c>int</c>-typed
    /// one, so a suite that asserts on one has said nothing about the other; culture-sensitive parsing
    /// in particular only shows up on the decimal one.
    /// </summary>
    /// <param name="configure">Configures the item's field. Runs after its default label, so it can override it.</param>
    /// <param name="configureCollection">
    /// Configures the <i>collection</i> rather than a field; runs last, after the label and item
    /// form. See <see cref="TextItemForm"/> for the full contract (#300).
    /// </param>
    public static IFormConfiguration<PricedBasketModel> DecimalItemForm(
    ...
    /// </para>
    /// </remarks>
    public static MixedItemModel NewMixedItems(params MixedItem[] rows)
    {
    ...
    /// </para>
    /// </remarks>
    public static IFormConfiguration<MixedItemModel> MultiFieldItemForm(
    ...
    /// </summary>
    public static TwoCollectionModel NewTwoCollections(string contact = "", string supplier = "") =>
        new()
        {
    ...
    /// </para>
    /// </remarks>
    public static IFormConfiguration<TwoCollectionModel> TwoCollectionItemForm(
    ...
    /// contract (#300).
    /// </param>
    public static IFormConfiguration<NamedOrderModel> RootFieldAndItemForm(
...
public sealed class PricedBasketModel
{
...
/// </summary>
public sealed class NamedOrderModel
{
...
public sealed class NamedOrderItem
{
    public string Name { get; set; } = string.Empty;
}
...
/// </summary>
public sealed class TwoCollectionModel
{
...
public sealed class MixedItemModel
{
...
/// text field, a numeric field, a checkbox and a date field side by side (#282).
/// <para>
