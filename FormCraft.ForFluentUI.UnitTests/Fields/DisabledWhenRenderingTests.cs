...
/// </summary>
public class DisabledWhenRenderingTests : FluentUITestBase
{
    [Fact]
    public void A_Condition_True_At_First_Render_Should_Disable_The_Control()
    {
        ...
        // Assert
        NameField(component).Disabled.ShouldBe(true);
    }

    [Fact]
    public async Task The_Control_Should_Follow_The_Condition_As_The_Watched_Field_Changes()
    {
        // Arrange
        var model = new LockModel { Mode = "open" };
        var component = RenderForm(model, f => f.DisabledWhen(m => m.Mode == "locked"));
        NameField(component).Disabled.ShouldBe(false);

        // Act + Assert - edit the watched field the way the input's binding would
        await component.InvokeAsync(() => ModeField(component).ValueChanged.InvokeAsync("locked"));
        component.WaitForAssertion(() => NameField(component).Disabled.ShouldBe(true));

        await component.InvokeAsync(() => ModeField(component).ValueChanged.InvokeAsync("open"));
        component.WaitForAssertion(() => NameField(component).Disabled.ShouldBe(false));
    }
    ...
    private static FluentTextInput NameField(IRenderedComponent<FormCraftComponent<LockModel>> component) =>
        Field(component, "Name");

    private static FluentTextInput ModeField(IRenderedComponent<FormCraftComponent<LockModel>> component) =>
        Field(component, "Mode");

    private static FluentTextInput Field(IRenderedComponent<FormCraftComponent<LockModel>> component, string label) =>
        component.FindComponents<FluentTextInput>()
            .Single(f => f.Instance.Label == label)
            .Instance;
